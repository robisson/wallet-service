 ...
 * 
 * <p>This service implements business logic that doesn't naturally fit within
...
public class WalletDomainService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(WalletDomainService.class);
 ...
 * </ul>
    ...
    public void transfer(WalletId fromWalletId, WalletId toWalletId, Money amount) {
        ...
        String transferId = "transfer-" + fromWalletId.value() + "-" + toWalletId.value() + "-" + System.currentTimeMillis();
        AuditInfo auditInfo = AuditInfo.builder()
            .requestId(transferId)
            .timestamp(Instant.now())
            .build();
            
        Transaction outTransaction = new Transaction(fromWalletId, Transaction.Type.TRANSFER_OUT, amount, toWalletId, auditInfo);
        Transaction inTransaction = new Transaction(toWalletId, Transaction.Type.TRANSFER_IN, amount, fromWalletId, auditInfo);
        ...
        logger.info("Transfer executed successfully from wallet: {} to wallet: {} amount: {} balances: {} -> {}, {} -> {}", 
                   fromWalletId.value(), toWalletId.value(), amount.amount(),
